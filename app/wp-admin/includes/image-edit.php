...
function wp_stream_image( $image, $mime_type, $attachment_id ) {
	error_log("wp_stream_image. Mime type: $mime_type");
	if ( $image instanceof WP_Image_Editor ) {
		 ...
		 */
		error_log("wp_stream_image. About to apply filter image_editor_save_pre");
		$image = apply_filters( 'image_editor_save_pre', $image, $attachment_id );

		error_log("wp_stream_image. About to stream image");
		$xxx_result = $image->stream( $mime_type );
		if ( is_wp_error( $xxx_result ) ) {
			error_log("wp_stream_image. Image stream failed with error: " . print_r($xxx_result, true));
			return false;
		}
...
function stream_preview_image( $post_id ) {
	error_log("stream_preview_image. Post: $post_id");
	$post = get_post( $post_id );

	wp_raise_memory_limit( 'admin' );

	error_log("stream_preview_image. 2");

	$xxx_path = _load_image_to_edit_path( $post_id );
	error_log("stream_preview_image. Path to edit: $xxx_path");

	$img = wp_get_image_editor( _load_image_to_edit_path( $post_id ) );

	error_log("stream_preview_image. 3");
	if ( is_wp_error( $img ) ) {
		error_log("stream_preview_image. Could not get image editor");
		return false;
	}

	error_log("stream_preview_image. 4");
	...
	error_log("stream_preview_image. 5");

...
class S3_Uploads_Stream_Wrapper
{
	...
	public static function register( S3ClientInterface $client, $protocol = 's3', CacheInterface $cache = null ) {
		...
		error_log("S3_Uploads_Stream_Wrapper.register");
	...
	{
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#stream_open. path: $path, mode: $mode");
		...
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#stream_open. Passed validation");
	...
	{
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#stream_read. count: $count. PHP_INT_MAX: " . PHP_INT_MAX);
		return $this->body->read($count);
	}

	public function stream_seek($offset, $whence = SEEK_SET)
	{
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#stream_seek. offset: $offset. size: $this->size");
		// if( $offset > 1000000 ) {
		// 	$offset = 1000000;
		// 	error_log("-------------------------> S3_Uploads_Stream_Wrapper#stream_seek. Cannot seek; offset exceeds threshold; resetting to $offset");
		// }
	...
	{
			error_log("-------------------------> S3_Uploads_Stream_Wrapper#stream_stat");
	...
	{
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#url_stat. path: $path");
		$this->initProtocol($path);
		...
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#url_stat. path 2: $path");

		// Check if this path is in the url_stat cache
		if ($value = $this->getCacheStorage()->get($path)) {
			error_log("-------------------------> S3_Uploads_Stream_Wrapper#url_stat. Cache hit.");
			return $value;
		}
		...
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#url_stat. Cache miss.");
	...
	{
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#createStat. Path: $path");
		$this->initProtocol($path);
		$parts = $this->withPath($path);

		error_log("-------------------------> S3_Uploads_Stream_Wrapper#createStat. Parts: " . print_r($parts, true));

		if (!$parts['Key']) {
			error_log("-------------------------> S3_Uploads_Stream_Wrapper#createStat. About to stat directory");
			return $this->statDirectory($parts, $path, $flags);
		}

		error_log("-------------------------> S3_Uploads_Stream_Wrapper#createStat. Did not stat directory");

		return $this->boolCall(function () use ($parts, $path) {
			error_log("-------------------------> S3_Uploads_Stream_Wrapper#createStat. use handler.");

			try {
				$result = $this->getClient()->headObject($parts);

				error_log("-------------------------> S3_Uploads_Stream_Wrapper#createStat. result: " . print_r($result, true));
				...
				// error_log("-------------------------> S3_Uploads_Stream_Wrapper#createStat. Exception caught: " . print_r($e, true));
	...
	{
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#getOption. name: $name");
	...
	{
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#openReadStream. Starting...");
		...
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#openReadStream. Content length: $content_length");
		error_log("-------------------------> S3_Uploads_Stream_Wrapper#openReadStream. Body: " . print_r($this->body, true) );

		// Wrap the body in a caching entity body if seeking is allowed
		if ($this->getOption('seekable') && !$this->body->isSeekable()) {
			// $local_stream_options = array("size" => $this->size);
			// $local_stream = new Stream(fopen('php://temp', 'r+'), $local_stream_options);
			// error_log("-------------------------> S3_Uploads_Stream_Wrapper#openReadStream. Created local stream from options: " . print_r($local_stream_options, true));

			$this->body = new CachingStream($this->body);
			error_log("-------------------------> S3_Uploads_Stream_Wrapper#openReadStream. Stored CachingStream");
		}

		error_log("-------------------------> S3_Uploads_Stream_Wrapper#openReadStream. Returning true");
		return true;
	}
	...
	private function boolCall(callable $fn, $flags = null)
	{
		try {
			...
			$msg = $e->getMessage();
			error_log("-------------------------> S3_Uploads_Stream_Wrapper#boolCall. Exception caught: $msg");
